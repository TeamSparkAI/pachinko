...
export default function ServerDetailPage({ params }: { params: { serverId: string } }) {
  ...
  // Create client helper when server changes (host/gateway settings are not used for connections)
  useEffect(() => {
    ...
    const setupHelper = async () => {
      if (server) {
        try {
          ...
          currentHelper = await McpClientHelper.createForServer(server, null);
          setClientHelper(currentHelper);
        } catch (err) {
