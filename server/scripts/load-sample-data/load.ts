...
async function loadSampleData() {
    try {
        ...
        for (const serverName of serverNames) {
            ...
            if ('catalogId' in server) {
                delete (server as { catalogId?: string }).catalogId;
            }
        ...
        while (processingDate < now) {
            ...
            const messageCount = await processMessages(filterContext, data.server, data.messages, processingDate);
