...
export class SqliteServerModel extends ServerModel {
    ...
    private deserializeServerRow(server: ServerRow): ServerData {
        return {
            ...server,
    ...
    async findById(serverId: number): Promise<ServerData | null> {
        ...
        return this.deserializeServerRow(result.rows[0]);
    }
    ...
    async list(filter: ServerFilter, pagination: ServerPagination): Promise<ServerListResult> {
        ...
        const deserializedServers = servers.rows.map(server => this.deserializeServerRow(server));
    ...
    async getByIds(serverIds: number[]): Promise<ServerData[]> {
        ...
        return result.rows.map(server => this.deserializeServerRow(server));
    }
} 