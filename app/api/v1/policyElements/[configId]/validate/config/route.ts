...
export async function POST(
    ...
    try {
        ...
        if (elementClass.configValidator) {
            const validation = elementClass.configValidator(config);
        ...
        logger.error('Error validating policy element config:', error);
        return JsonResponse.errorResponse(500, 'Failed to validate configuration');
    }
}
